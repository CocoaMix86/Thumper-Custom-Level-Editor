...
namespace Thumper_Custom_Level_Editor.Editor_Panels
{
    public partial class DockProjectExplorer : EditorBaseSub
    {
        ...
        public static List<TreeNode> selectedNodes = new();
        ...
        private void existingItemToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK) {
                ...
                if (ProjectExplorer.TryGetFile(filetocopy.Name, out ProjectItem foundfile)) {
                ...
                if (File.Exists($"{ProjectExplorer.AllFiles[selectedNodes[0]].Folder.FullName}\\{filetocopy.Name}")) {
                    MessageBox.Show($"A filed named {filetocopy.Name} already exists in folder {selectedNodes[0].Text}. Operation cancelled", "Thumper Custom Level Editor");
                    return;
                }
        ...
        private void toolstripFileDuplicate_Click(object sender, EventArgs e)
        {
            ...
            ProjectExplorer.CreateTreeView();
            ChangeSelection(treeView1.FlattenTree().Where(x => x.FullPath == $"{targetpath}\\{Path.GetFileName(dest)}").ToList(), selectedNodes.ToList(), treeView1.BackColor);
        ...
        private void treeView1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (control && e.Button != MouseButtons.Right) {
                ...
                ChangeSelection(addedNodes, removedNodes, treeView1.BackColor);
            }
        ...
        public static void ChangeSelection(List<TreeNode> addedNodes, List<TreeNode> removedNodes, Color BackColor)
        {
        ...
        */
        ...
        private void folderToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (path == null) {
                ...
                int newfolders = new DirectoryInfo(path).GetDirectories().Where(x => x.Name.StartsWith("New Folder")).Count();
                Directory.CreateDirectory($"{path}\\New Folder{(newfolders > 0 ? $" {newfolders}" : "")}");
            }
            selectedNodes[^1].Expand();
            ProjectExplorer.CreateTreeView();
        }
